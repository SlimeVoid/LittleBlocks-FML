...
public class EntityItemLittleBlocksCollectionRenderer extends RenderItem {
	...
	public void doRenderItem(EntityItemLittleBlocksCollection collection, double x, double y, double z, float f1, float f2) {
		GL11.glPushMatrix();
		ItemStack itemstack = new ItemStack(Block.chest.blockID, 1, 0);
			Random rand = new Random();
			this.random.put(itemstack, rand);
			this.random.get(itemstack).setSeed(187L);
			if (itemstack.getItem() != null) {
				float f3 = shouldBob() ? MathHelper
						.sin((collection.age + f2) / 10.0F + collection.hoverStart) * 0.1F + 0.1F : 0F;
				...
				if (ForgeHooksClient.renderEntityItem(
						...
						f3,
						f4,
						this.random.get(itemstack),
						renderManager.renderEngine,
						renderBlocks)) {
					;
				} else if (itemstack.getItem() instanceof ItemBlock && RenderBlocks
					...
					this.loadTexture(Block.blocksList[itemstack.itemID]
							.getTextureFile());
					float var22 = 0.25F;
					renderType = Block.blocksList[itemstack.itemID]
							.getRenderType();
					...
					if (itemstack.getItem().requiresMultipleRenderPasses()) {
						...
						for (renderPass = 0; renderPass <= itemstack
								.getItem()
									.getRenderPasses(itemstack.getItemDamage()); ++renderPass) {
							this.loadTexture(Item.itemsList[itemstack.itemID]
									.getTextureFile());
							this.random.get(itemstack).setSeed(187L);
							renderType = itemstack.getItem().getIconIndex(
									itemstack,
									renderPass);
							colourMultiplier = 1.0F;

							if (this.field_77024_a) {
								int var18 = Item.itemsList[itemstack.itemID]
						...
						if (this.field_77024_a) {
							renderType = Item.itemsList[itemstack.itemID]
	...
	private void renderItem(EntityItem par1EntityItem, ItemStack itemstack, int par2, int par3, float par4, float par5, float par6, float par7) {
		...
		if (this.renderManager.options.fancyGraphics) {
			...
			if (field_82407_g) {
				...
				GL11
						.glRotatef(
								((par1EntityItem.age + par4) / 20.0F + par1EntityItem.hoverStart) * (180F / (float) Math.PI),
								0.0F,
								1.0F,
								0.0F);
			}
			...
			for (int var20 = 0; var20 < miniCount; ++var20) {
				...
				if (itemstack != null && itemstack.hasEffect()) {
					...
					this.renderManager.renderEngine
							.bindTexture(this.renderManager.renderEngine
			...
			for (int var25 = 0; var25 < par3; ++var25) {
				...
				if (!field_82407_g) {
					GL11.glRotatef(
							180.0F - this.renderManager.playerViewY,
							0.0F,
							1.0F,
							0.0F);
				}
