...
public class ConfigurationLib {
	...
	public static void CommonConfig(File configFile) {
		...
		String disallowedBlockIDs[] = configuration
				.get(Configuration.CATEGORY_GENERAL, "disallowedBlockIDs", "")
				.getString().split("\\;", -1);
		for (int i = 0; i < disallowedBlockIDs.length; i++) {
			if (!disallowedBlockIDs[i].isEmpty()) {
				BlockUtil.registerDisallowedBlockID(Integer
						.valueOf(disallowedBlockIDs[i]));
			}
		}
		String disallowedItemIDs[] = configuration
				.get(Configuration.CATEGORY_GENERAL, "disallowedItemIDs", "")
				.getString().split("\\;", -1);
		for (int i = 0; i < disallowedItemIDs.length; i++) {
			if (!disallowedItemIDs[i].isEmpty()) {
				BlockUtil.registerDisallowedItemID(Integer
						.valueOf(disallowedItemIDs[i]));
			}
		}

		littleChunkID = configuration.get(Configuration.CATEGORY_GENERAL,
				"littleChunkID", 1150).getInt();
		littleBlocksWandID = configuration.get(Configuration.CATEGORY_GENERAL,
				"littleBlocksWandID", 29999).getInt();
		littleBlocksCollectionID = configuration.get(
				Configuration.CATEGORY_GENERAL, "littleBlocksCollectionID",
				EntityRegistry.findGlobalUniqueEntityId()).getInt();
		littleBlocksClip = configuration.get(Configuration.CATEGORY_GENERAL,
				"littleBlocksClip", true).getBoolean(true);
		littleBlocksForceUpdate = configuration.get(
				Configuration.CATEGORY_GENERAL, "littleBlocksForceUpdate",
				false).getBoolean(false);
		renderingMethod = configuration.get(Configuration.CATEGORY_GENERAL,
				"renderingMethod", 0).getInt();
		renderType = RenderingRegistry.getNextAvailableRenderId();
		loggerLevel = configuration.get(Configuration.CATEGORY_GENERAL,
				"loggerLevel", "INFO").getString();

		int[] disallowedIds = configuration.get(Configuration.CATEGORY_GENERAL,
				"disallowedLittleDimensionIds", new int[] { 7, 20 })
				.getIntList();
		...
		pistonStickyIndex = configuration.get(CATEGORY_OVERRIDES,
				"pistonStickyIndex", pistonStickyIndex).getInt();
		pistonBaseIndex = configuration.get(CATEGORY_OVERRIDES,
				"pistonBaseIndex", pistonBaseIndex).getInt();

		doorWoodIndex = configuration.get(CATEGORY_OVERRIDES, "doorWoodIndex",
				doorWoodIndex).getInt();
		doorIronIndex = configuration.get(CATEGORY_OVERRIDES, "doorIronIndex",
				doorIronIndex).getInt();

		configuration.save();

		LoggerLittleBlocks.getInstance("LittleBlocksConfig").setFilterLevel(
				loggerLevel);
	}
