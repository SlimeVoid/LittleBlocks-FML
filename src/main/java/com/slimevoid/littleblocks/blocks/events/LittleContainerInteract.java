...
public class LittleContainerInteract {
		...
		if (!event.canInteractWith) {
			try {
				Field fields[] = event.entityPlayer.openContainer.getClass()
						.getDeclaredFields();
				GenericCanInteract(event, fields,
	...
	private void GenericCanInteract(PlayerOpenContainerEvent event,
			Field fields[], Object datasource) {
		for (int i = 0; i < fields.length && event.getResult() != Result.ALLOW; i++) {
			try {
				...
				if (fields[i].get(datasource) instanceof TileEntity) {
					...
					IBlockAccess world = (IBlockAccess) fields[i]
							.get(datasource);
					WorldCanInteract(event, fields[i], world, fields, i,
							datasource);
					break;

				} else if (fields[i].get(datasource) instanceof IInventory) {
					GenericCanInteract(event, fields[i].get(datasource)
		...
		if (event.getResult() != Result.ALLOW
			...
			FakePlayer fakePlayer = new FakePlayer(
					(WorldServer) event.entityPlayer.worldObj, new GameProfile(
							String.valueOf(event.entityPlayer.getEntityId()),
							CoreLib.MOD_CHANNEL));
	...
	private void TileEntityCanInteract(PlayerOpenContainerEvent event,
			TileEntity tile) {
		if (Math.pow(Math.pow((tile.xCoord >> 3) - event.entityPlayer.posX, 2)
				+ Math.pow((tile.yCoord >> 3) - event.entityPlayer.posY, 2)
				+ Math.pow((tile.zCoord >> 3) - event.entityPlayer.posZ, 2), .5) <= 4) {
			FakePlayer fakePlayer = new FakePlayer(
					(WorldServer) event.entityPlayer.worldObj, new GameProfile(
							String.valueOf(event.entityPlayer.getEntityId()),
							CoreLib.MOD_CHANNEL));
	...
	private void WorldCanInteract(PlayerOpenContainerEvent event,
			Field worldField, IBlockAccess world, Field[] fields,
		...
		try {
			...
			if (fields.length > worldFieldIndex + 3) {
				...
				if (fields[worldFieldIndex + 1].get(datasource) instanceof Integer
					...
					if (Math.pow(
							Math.pow(
									(fields[worldFieldIndex + 1]
											.getInt(datasource) >> 3)
											- event.entityPlayer.posX, 2)
									+ Math.pow(
											(fields[worldFieldIndex + 2]
													.getInt(datasource) >> 3)
													- event.entityPlayer.posY,
											2)
									+ Math.pow(
											(fields[worldFieldIndex + 3]
													.getInt(datasource) >> 3)
													- event.entityPlayer.posZ,
											2), .5) <= 4) {
						fakePlayer.posX = fields[worldFieldIndex + 1]
								.getInt(datasource);
						fakePlayer.posY = fields[worldFieldIndex + 2]
								.getInt(datasource);
						fakePlayer.posZ = fields[worldFieldIndex + 3]
								.getInt(datasource);
						checked = true;
					}
				}
