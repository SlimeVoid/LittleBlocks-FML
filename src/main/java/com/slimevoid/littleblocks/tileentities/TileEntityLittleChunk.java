...
public class TileEntityLittleChunk extends TileEntity implements ILittleBlocks {
	...
	private NibbleArray blockMetadataArray = new NibbleArray(
			this.blockLSBArray.length, 4);
	...
	public void updateContainingBlockInfo() {
		...
		this.getLittleWorld().activeChunkPosition(
				new ChunkPosition(this.xCoord, this.yCoord, this.zCoord), true);
	}
	...
	public int getLightlevel() {
			...
			for (int y = 0; y < size; y++) {
				for (int z = 0; z < size; z++) {
					if (this.getBlockByExtId(x, y, z) == null) {
						...
						lightcount[this.getBlockByExtId(x, y, z).getLightValue(
								this.getLittleWorld(), blockX + x, blockY + y,
								blockZ + z)]++;
					}
				}
			}
		...
		for (int i = 15; i > 0; i--) {
			if (lightcount[i] > 0) {
				int templightlvl = lightcount[i] >= size ? i : MathHelper
						.ceiling_double_int(((double) i / (double) size)
								* (double) lightcount[i]);
				if (templightlvl > calculatedLightLevel)
					calculatedLightLevel = templightlvl;
			}
		}
	...
	public int getBlockMetadata(int x, int y, int z) {
		if (x >= size | y >= size | z >= size) {
			if (this.worldObj.getBlock(xCoord + (x >= size ? 1 : 0), yCoord
					+ (y >= size ? 1 : 0), zCoord + (z >= size ? 1 : 0)) == ConfigurationLib.littleChunk) {
				TileEntityLittleChunk tile = (TileEntityLittleChunk) this.worldObj
						.getTileEntity(xCoord + (x >= size ? 1 : 0), yCoord
								+ (y >= size ? 1 : 0), zCoord
								+ (z >= size ? 1 : 0));
				return tile.getBlockMetadata(x >= size ? x - size : x,
						y >= size ? y - size : y, z >= size ? z - size : z);
			}
			if (this.worldObj.getBlock(xCoord + (x >= size ? 1 : 0), yCoord
					+ (y >= size ? 1 : 0), zCoord + (z >= size ? 1 : 0)) == Blocks.air) {
				return 0;
			}
			return -1;
		} else if (x < 0 | z < 0 | y < 0) {
			if (this.worldObj.getBlock(xCoord - (x < 0 ? 1 : 0), yCoord
					- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0)) == ConfigurationLib.littleChunk) {
				TileEntityLittleChunk tile = (TileEntityLittleChunk) this.worldObj
						.getTileEntity(xCoord - (x < 0 ? 1 : 0), yCoord
								- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0));
				return tile.getBlockMetadata(x < 0 ? x + size : x, y < 0 ? y
						+ size : y, z < 0 ? z + size : z);
			}
			if (this.worldObj.getBlock(xCoord - (x < 0 ? 1 : 0), yCoord
					- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0)) == Blocks.air) {
				return 0;
			}
			return -1;
		} else {
			...
			if (this.worldObj.getBlock(xCoord + (x >= size ? 1 : 0), yCoord
				...
				TileEntityLittleChunk tile = (TileEntityLittleChunk) this.worldObj
						.getTileEntity(xCoord + (x >= size ? 1 : 0), yCoord
								+ (y >= size ? 1 : 0), zCoord
								+ (z >= size ? 1 : 0));
				return tile.getBlock(x >= size ? x - size : x, y >= size ? y
		...
		if (x >= size | y >= size | z >= size) {
			...
			if (this.worldObj.getBlock(xCoord - (x < 0 ? 1 : 0), yCoord
				...
				TileEntityLittleChunk tile = (TileEntityLittleChunk) this.worldObj
						.getTileEntity(xCoord - (x < 0 ? 1 : 0), yCoord
								- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0));
				return tile.getBlockLightValue(x < 0 ? x + size : x, y < 0 ? y
						+ size : y, z < 0 ? z + size : z, side);
			} else {
				return this.worldObj.getBlockLightValue(this.xCoord,
						this.yCoord, this.zCoord);
			}
			...
			if (this.worldObj.getBlock(xCoord + (x >= size ? 1 : 0), yCoord
				...
				TileEntityLittleChunk tile = (TileEntityLittleChunk) this.worldObj
						.getTileEntity(xCoord + (x >= size ? 1 : 0), yCoord
								+ (y >= size ? 1 : 0), zCoord
								+ (z >= size ? 1 : 0));
				tile.setLightValue(x >= size ? x - size : x, y >= size ? y
						- size : y, z >= size ? z - size : z, value);
			}
		} else if (x < 0 | z < 0 | y < 0) {
			if (this.worldObj.getBlock(xCoord - (x < 0 ? 1 : 0), yCoord
					- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0)) == ConfigurationLib.littleChunk) {
				TileEntityLittleChunk tile = (TileEntityLittleChunk) this.worldObj
						.getTileEntity(xCoord - (x < 0 ? 1 : 0), yCoord
								- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0));
				tile.setLightValue(x < 0 ? x + size : x, y < 0 ? y + size : y,
						z < 0 ? z + size : z, value);
			}
		}
	...
	public boolean setBlockMetadata(int x, int y, int z, int metadata) {
		if (x >= size | y >= size | z >= size) {
			if (this.worldObj.getBlock(xCoord + (x >= size ? 1 : 0), yCoord
					+ (y >= size ? 1 : 0), zCoord + (z >= size ? 1 : 0)) == Blocks.air) {
				this.worldObj.setBlock(xCoord + (x >= size ? 1 : 0), yCoord
						+ (y >= size ? 1 : 0), zCoord + (z >= size ? 1 : 0),
						ConfigurationLib.littleChunk, 0, 0x02);
			}
			if (this.worldObj.getBlock(xCoord + (x >= size ? 1 : 0), yCoord
					+ (y >= size ? 1 : 0), zCoord + (z >= size ? 1 : 0)) == ConfigurationLib.littleChunk) {
				TileEntityLittleChunk tile = (TileEntityLittleChunk) this.worldObj
			...
			if (this.worldObj.getBlock(xCoord - (x < 0 ? 1 : 0), yCoord
					- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0)) == Blocks.air) {
				this.worldObj.setBlock(xCoord - (x < 0 ? 1 : 0), yCoord
						- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0),
						ConfigurationLib.littleChunk, 0, 0x02);
			}
			if (this.worldObj.getBlock(xCoord - (x < 0 ? 1 : 0), yCoord
				...
				TileEntityLittleChunk tile = (TileEntityLittleChunk) this.worldObj
						.getTileEntity(xCoord - (x < 0 ? 1 : 0), yCoord
								- (y < 0 ? 1 : 0), zCoord - (z < 0 ? 1 : 0));
				return tile.setBlockMetadata(x < 0 ? x + size : x, y < 0 ? y
						+ size : y, z < 0 ? z + size : z, metadata);
			}
		}
	...
	public TileEntity getTileEntityUnsafe(int x, int y, int z) {
		...
		TileEntity tileentity = (TileEntity) this.chunkTileEntityMap
				.get(chunkposition);
	...
	public boolean setBlockIDWithMetadata(int x, int y, int z, Block block,
			int metadata) {
		if (x >= size | y >= size | z >= size) {
			if (this.worldObj.getBlock(xCoord + (x >= size ? 1 : 0), yCoord
					+ (y >= size ? 1 : 0), zCoord + (z >= size ? 1 : 0)) == Blocks.air) {
			...
			if (this.worldObj.getBlock(xCoord - (x < 0 ? 1 : 0), yCoord
				...
				return tile.setBlockIDWithMetadata(x < 0 ? x + size : x,
						y < 0 ? y + size : y, z < 0 ? z + size : z, block,
						metadata);
			}
		}
		...
		if (lastBlock == block && lastData == metadata) {
			...
			if (!this.worldObj.isRemote) {
				lastBlock.breakBlock((World) this.getLittleWorld(),
						(this.xCoord << 3) + x, (this.yCoord << 3) + y,
						(this.zCoord << 3) + z, lastBlock, lastData);
			} else if (lastBlock.hasTileEntity(lastData)) {
				TileEntity te = this.getTileEntityUnsafe(x, y, z);
				if (te != null
						&& te.shouldRefresh(lastBlock, block, lastData,
								metadata, (World) this.getLittleWorld(),
								(this.xCoord << 3) + x, (this.yCoord << 3) + y,
								(this.zCoord << 3) + z)) {
					((World) this.getLittleWorld()).removeTileEntity(
							(this.xCoord << 3) + x, (this.yCoord << 3) + y,
							(this.zCoord << 3) + z);
				}
			}
	...
	protected void setLittleWorldObjs() {
		...
		while (ticks.hasNext()) {
			...
			((World) this.getLittleWorld())
					.func_147446_b/* scheduleBlockUpdateFromLoad */(
	...
	public void setChunkBlockTileEntity(int x, int y, int z, TileEntity tile) {
		...
		if (block != null
				&& block.hasTileEntity(this.getBlockMetadata(x, y, z))) {
	...
	public void onChunkUnload() {
		...
		while (tiles.hasNext()) {
			...
			((World) this.getLittleWorld())
					.func_147457_a/* markTileEntityForDespawn */(tile);
		}
	}
	...
	public TileEntity getChunkTileEntity(int x, int y, int z) {
		...
		if (tileentity == null) {
			...
			if (tileentity == null) {
				tileentity = littleBlock.createTileEntity(
						(World) this.getLittleWorld(), meta);
				((World) this.getLittleWorld()).setTileEntity(
						(this.xCoord << 3) + x, (this.yCoord << 3) + y,
						(this.zCoord << 3) + z, tileentity);
			}
			tileentity = (TileEntity) this.chunkTileEntityMap
					.get(chunkposition);
		}
		return tileentity;
	}
	...
	private void addTileEntity(TileEntity tile) {
		...
		int y = tile.yCoord & 7;
		int z = tile.zCoord & 7;
		this.setChunkBlockTileEntity(x, y, z, tile);
	}
				...
				for (int y = 0; y < size; y++) {
					for (int z = 0; z < size; z++) {
						if (this.getBlockByExtId(x, y, z) == null) {
							...
							ChunkPosition chunkposition = new ChunkPosition(x,
									y, z);
							this.chunkTileEntityMap.remove(chunkposition);
						}
					}
	...
	public void readFromNBT(NBTTagCompound nbttagcompound) {
		...
		if (nbttagcompound.hasKey("Add", 7)) {
			this.blockMSBArray = new NibbleArray(
					nbttagcompound.getByteArray("Add"), 4);
		}
		this.blockMetadataArray = new NibbleArray(
		...
		if (tilesTag != null) {
			for (int i = 0; i < tilesTag.tagCount(); i++) {
				NBTTagCompound tileCompound = (NBTTagCompound) tilesTag
						.getCompoundTagAt(i);
		...
		if (nbttagcompound.hasKey("TileTicks")) {
			...
			if (tickList != null) {
				for (int i = 0; i < tickList.tagCount(); i++) {
					NBTTagCompound pendingTick = (NBTTagCompound) tickList
							.getCompoundTagAt(i);
	...
	public void writeToNBT(NBTTagCompound nbttagcompound) {
		...
		List pendingUpdates = ((World) this.getLittleWorld())
				.getPendingBlockUpdates(
						new Chunk((World) this.getLittleWorld(), this.xCoord,
								this.zCoord), false);
		if (pendingUpdates != null) {
			...
			while (pendingIterator.hasNext()) {
				NextTickListEntry nextticklistentry = (NextTickListEntry) pendingIterator
						.next();
				NBTTagCompound pendingUpdate = new NBTTagCompound();
				pendingUpdate
						.setInteger("i", Block.getIdFromBlock(nextticklistentry
								.func_151351_a()));
	...
	public Packet getDescriptionPacket() {
		...
		Packet packet = new S35PacketUpdateTileEntity(xCoord, yCoord, zCoord,
				0, nbttagcompound);
		return packet;
	}
					...
					if (littleBlock != Blocks.air) {
						...
						if (littleBlock != null) {
							if (littleBlock.rotateBlock(
									(World) this.getLittleWorld(),
									(this.xCoord << 3) + x, (this.yCoord << 3)
											+ y, (this.zCoord << 3) + z, axis)) {
							}
	...
	public void littleUpdateTick(ILittleWorld littleWorld, int updateLCG) {
		...
		if (block != null && block.getTickRandomly()) {
			block.updateTick((World) littleWorld, this.getX(x), this.getY(y),
					this.getZ(z), ((World) littleWorld).rand);
		}
	}
}
