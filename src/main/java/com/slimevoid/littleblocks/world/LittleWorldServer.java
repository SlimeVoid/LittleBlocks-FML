...
public class LittleWorldServer extends WorldServer implements ILittleWorld {
	...
	public LittleWorldServer(World referenceWorld,
			MinecraftServer minecraftServer, ISaveHandler iSaveHandler,
			String par3Str, int par4, WorldSettings par5WorldSettings,
			Profiler par6Profiler) {
		super(minecraftServer, iSaveHandler, par3Str, par4, par5WorldSettings,
				par6Profiler);
	 ...
	 */
	@Override
	public boolean isBlockTickScheduledThisTick(int x, int y, int z,
			Block blockId) {
		return this.getLittleWorld().isBlockTickScheduledThisTick(x, y, z,
				blockId);
	}
	...
	public Explosion newExplosion(Entity entity, double x, double y, double z,
		...
		return this.getLittleWorld().newExplosion(entity, x, y, z, strength,
				isFlaming, isSmoking);
	}
	 ...
	 */
	@Override
	public void addBlockEvent(int x, int y, int z, Block blockID, int eventID,
			int eventParam) {
		this.getLittleWorld().addBlockEvent(x, y, z, blockID, eventID,
				eventParam);
	}
	 ...
	 */
	@Override
	public void scheduleBlockUpdate(int x, int y, int z, Block blockId,
			int tickRate) {
	 ...
	 */
	@Override
	public void scheduleBlockUpdateWithPriority(int x, int y, int z,
			Block blockId, int tickRate, int someValue) {
		this.getLittleWorld().scheduleBlockUpdateWithPriority(x, y, z, blockId,
				tickRate, someValue);
	}

	@Override
	public void func_147446_b/* scheduleBlockUpdateFromLoad */(int x, int y,
			int z, Block blockId, int tickRate, int par6) {
		this.getLittleWorld().func_147446_b/* scheduleBlockUpdateFromLoad */(
				x, y, z, blockId, tickRate, par6);
	}

	public int getSkyBlockTypeBrightness(EnumSkyBlock enumskyblock, int x,
			int y, int z) {
		return this.getLittleWorld().getSkyBlockTypeBrightness(enumskyblock, x,
				y, z);
	}
	...
	public float getLightBrightness(int x, int y, int z) {
		return this.getLittleWorld().getLightBrightness(x, y, z);
	}
	...
	public int getFullBlockLightValue(int x, int y, int z) {
		return this.getLittleWorld().getFullBlockLightValue(x, y, z);
	}

	@Override
	public boolean canBlockSeeTheSky(int x, int y, int z) {
		return this.getLittleWorld().canBlockSeeTheSky(x, y, z);
	}
	...
	public Block getBlock(int x, int y, int z) {
		return this.getLittleWorld().getBlock(x, y, z);
	}
	...
	@Override
	public boolean setBlock(int x, int y, int z, Block blockID, int newmeta,
			int update) {
	...
	public boolean setBlockMetadataWithNotify(int x, int y, int z, int newmeta,
			int update) {
		return this.getLittleWorld().setBlockMetadataWithNotify(x, y, z,
				newmeta, update);
	}
	...
	public boolean checkChunksExist(int minX, int minY, int minZ, int maxX,
			int maxY, int maxZ) {
		return this.getLittleWorld().checkChunksExist(minX, minY, minZ, maxX,
				maxY, maxZ);
	}
	...
	public TileEntity getTileEntity(int x, int y, int z) {
		return this.getLittleWorld().getTileEntity(x, y, z);
	}

	@Override
	public void setTileEntity(int x, int y, int z, TileEntity tileentity) {
		this.getLittleWorld().setTileEntity(x, y, z, tileentity);
	}
	...
	public void func_147457_a/* markTileEntityForDespawn */(TileEntity tileentity) {
		this.getLittleWorld().func_147457_a/* markTileEntityForDespawn */(
				tileentity);
	}

	@Override
	public void removeTileEntity(int x, int y, int z) {
		this.getLittleWorld().removeTileEntity(x, y, z);
	}

	@Override
	public boolean isSideSolid(int x, int y, int z, ForgeDirection side,
			boolean _default) {
		return this.getLittleWorld().isSideSolid(x, y, z, side, _default);
	}
	...
	public MovingObjectPosition func_147447_a/* rayTraceBlocks_do_do */(
		...
		return this.getLittleWorld().func_147447_a/* rayTraceBlocks_do_do */(
				Vec3, Vec31, flag, flag1, flag2);
	}
	...
	public EntityPlayer getClosestPlayer(double x, double y, double z,
		...
		EntityPlayer entityplayer = this.getParentWorld().getClosestPlayer(
				blockX >> 3, blockY >> 3, blockZ >> 3, distance);
		if (entityplayer != null) {
			FakePlayer player = new FakePlayer((WorldServer) this,
	...
	public EntityPlayer getClosestVulnerablePlayer(double x, double y,
		...
		return this.getLittleWorld().getClosestVulnerablePlayer(x, y, z,
				distance);
	}
	...
	public boolean updateLightByType(EnumSkyBlock enumSkyBlock, int x, int y,
			int z) {
		return this.getLittleWorld().updateLightByType(enumSkyBlock, x, y, z);
	}
	...
	@Override
	public boolean canPlaceEntityOnSide(Block blockId, int x, int y, int z,
			boolean flag, int side, Entity entityPlacing, ItemStack itemstack) {
		return this.getLittleWorld().canPlaceEntityOnSide(blockId, x, y, z,
				flag, side, entityPlacing, itemstack);
	}

	@Override
	public List getEntitiesWithinAABBExcludingEntity(Entity entity,
			AxisAlignedBB axisalignedbb, IEntitySelector entitySelector) {
		return this.getLittleWorld().getEntitiesWithinAABBExcludingEntity(
				entity, axisalignedbb, entitySelector);
	}
	...
	public boolean isOutSideLittleWorld(int x, int y, int z) {
		return this.getLittleWorld().isOutSideLittleWorld(x, y, z);
	}
	...
	public void activeChunkPosition(ChunkPosition chunkposition, boolean forced) {
		this.getLittleWorld().activeChunkPosition(chunkposition, forced);
	}
}
