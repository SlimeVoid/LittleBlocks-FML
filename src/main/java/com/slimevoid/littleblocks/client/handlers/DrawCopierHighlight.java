...
import com.slimevoid.littleblocks.core.lib.TextureLib;
...
public class DrawCopierHighlight {
	...
	public void drawInWorldCopierOverlay(DrawBlockHighlightEvent event) {
		...
		if (this.shouldDoDraw(world, event.target.blockX, event.target.blockY,
				event.target.blockZ)) {
			xShift = 0;
			yShift = 0;
			zShift = 0;
		...
		ForgeDirection sideHit = ForgeDirection
				.getOrientation(event.target.sideHit);
		...
		Block blockID = world.getBlock(event.target.blockX + (int) xShift,
				event.target.blockY + (int) yShift, event.target.blockZ
						+ (int) zShift);
		if (!(blockID instanceof IFluidBlock)) {
			...
			for (int i = 0; i < 6; i++) {
				...
				GL11.glPushMatrix();
				GL11.glTranslated(-iPX + x + xShift, -iPY + y + yShift, -iPZ
						+ z + zShift);
				GL11.glScalef(1F * xScale, 1F * yScale, 1F * zScale);
				GL11.glRotatef(90, forgeDir.offsetX, forgeDir.offsetY,
						forgeDir.offsetZ);
				GL11.glTranslated(0, 0, 0.5f * zCorrection);
				GL11.glClear(GL11.GL_DEPTH_BUFFER_BIT);
				renderPulsingQuad(
						FMLClientHandler.instance().getClient().renderEngine,
						TextureLib.WAND_OVERLAY, 0.75F);
				GL11.glPopMatrix();
			}
	...
	private boolean shouldDoDraw(World world, int blockX, int blockY, int blockZ) {
		...
		return block == ConfigurationLib.littleChunk
				|| block.isReplaceable(world, blockX, blockY, blockZ);
	}

	public static void renderPulsingQuad(TextureManager renderEngine,
			ResourceLocation wandOverlay, float maxTransparency) {
