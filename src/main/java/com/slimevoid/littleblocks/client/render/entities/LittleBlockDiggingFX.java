...
public class LittleBlockDiggingFX extends EntityDiggingFX {
	...
	public LittleBlockDiggingFX(World par1World, double par2, double par4,
			double par6, double par8, double par10, double par12,
			Block par14Block, int par15) {
		super(par1World, par2, par4, par6, par8, par10, par12, par14Block,
				par15, par1World.rand.nextInt(6));
		this.particleScale /= ConfigurationLib.littleBlocksSize;
	}

	public static boolean doBlockDestroyEffects(World world, int x, int y,
			int z, int meta, EffectRenderer effectRenderer,
						...
						for (int l1 = 0; l1 < b0; ++l1) {
							...
							try {
								LittleBlockDiggingFX particle = new LittleBlockDiggingFX(
										world, d0, d1, d2, d0 - (double) x
												- 0.5D, d1 - (double) y - 0.5D,
										d2 - (double) z - 0.5D, littleBlock,
										littleMeta);
								effectRenderer.addEffect(particle
										.applyColourMultiplier(x, y, z));
							} catch (ArrayIndexOutOfBoundsException e) {
								LoggerLittleBlocks
										.getInstance(CoreLib.MOD_ID)
										.write(true,
	...
	public static boolean doBlockHitEffects(World world,
		...
		if (littleWorld != null) {
			...
			if (littleBlock != null) {
				...
				try {
					LittleBlockDiggingFX particle = new LittleBlockDiggingFX(
							world, d0, d1, d2, 0.0D, 0.0D, 0.0D, littleBlock,
							littleMeta);
					effectRenderer.addEffect(particle
							.applyColourMultiplier(x, y, z)
					...
					LoggerLittleBlocks.getInstance(CoreLib.MOD_ID).write(
							true,
