...
public class LittleBlocksLittleRenderer {
	...
	public void renderLittleBlocks(IBlockAccess iblockaccess, int x, int y,
			int z) {
			...
			for (LittleBlockToRender littleBlockToRender : this.littleBlocksToRender) {
				try {
					this.renderBlocks.renderBlockByRenderType(
							littleBlockToRender.block, littleBlockToRender.x,
							littleBlockToRender.y, littleBlockToRender.z);
				} catch (IllegalArgumentException e) {
					LoggerLittleBlocks.getInstance("LittleBlocksMod").write(
							true,
