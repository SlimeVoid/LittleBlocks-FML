...
public class BlockUtil {
	...
	@SideOnly(Side.CLIENT)
	public static void setLittleController(LittlePlayerController controller,
	...
	public static LittlePlayerController getLittleController() {
		if (littlePlayerController == null) {
			...
			setLittleController(new LittlePlayerController(FMLClientHandler
					.instance().getClient(),
					(NetHandlerPlayClient) FMLClientHandler.instance()
							.getClientPlayHandler()), world.getWorldInfo()
					.getGameType());
		}
		return littlePlayerController;
	}

	public static ItemInLittleWorldManager getLittleItemManager(
			EntityPlayerMP entityplayer, World world) {
		if (itemInLittleWorldManagers.containsKey(entityplayer)
				&& itemInLittleWorldManagers.get(entityplayer).theWorld
						.equals(world)) {
	...
	private static ItemInLittleWorldManager setLittleItemManagerForPlayer(
			EntityPlayerMP entityplayer) {
		itemInLittleWorldManagers.put(entityplayer,
				new ItemInLittleWorldManager(entityplayer.worldObj,
						entityplayer));
		return itemInLittleWorldManagers.get(entityplayer);
	}
	...
	private static void registerDisallowedBlockTick(
	...
	private static void registerDisallowedBlock(
			Class<? extends Block> blockClass) {
		if (blockClass != null) {
	...
	private static void registerDisallowedTile(
	...
	public static boolean isLittleChunk(World world, int x, int y, int z) {
		if (world instanceof ILittleWorld) {
			return ((ILittleWorld) world).getParentWorld().getBlock(x >> 3,
					y >> 3, z >> 3) == ConfigurationLib.littleChunk;
		}
		return false;
	}
	...
	public static void onServerBlockActivated(World world,
			EntityPlayer entityplayer, ItemStack stack, int x, int y, int z,
		...
		if (side == 255) {
			...
			PlayerInteractEvent event = ForgeEventFactory.onPlayerInteract(
					entityplayer, PlayerInteractEvent.Action.RIGHT_CLICK_AIR,
					0, 0, 0, -1);
			if (event.useItem != Event.Result.DENY) {
				getLittleItemManager((EntityPlayerMP) entityplayer, world)
						.tryUseItem(entityplayer, world, itemstack);
			}
			...
			// dist *= dist;
			if (!mcServer.isBlockProtected(
					((ILittleWorld) world).getParentWorld(), x >> 3, y >> 3,
					z >> 3, entityplayer)) {
				getLittleItemManager((EntityPlayerMP) entityplayer, world)
						.activateBlockOrUseItem(entityplayer, world, itemstack,
		...
		if (itemstack == null || itemstack.getMaxItemUseDuration() == 0) {
			...
			Slot slot = entityplayer.openContainer.getSlotFromInventory(
					entityplayer.inventory, entityplayer.inventory.currentItem);
			entityplayer.openContainer.detectAndSendChanges();
			((EntityPlayerMP) entityplayer).isChangingQuantityOnly = false;

			if (!ItemStack.areItemStacksEqual(
					entityplayer.inventory.getCurrentItem(), stack)) {
				((EntityPlayerMP) entityplayer).playerNetServerHandler
						.sendPacket(new S2FPacketSetSlot(
								entityplayer.openContainer.windowId,
								slot.slotNumber, entityplayer.inventory
										.getCurrentItem()));
			}
		}
	}

	private static void checkPlacement(World world, EntityPlayer entityplayer,
