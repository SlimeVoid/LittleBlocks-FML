...
public class PacketLib {
	...
	public static void registerClientPacketHandlers() {
		...
		handler.getPacketHandler(PacketIds.LOGIN).registerClientExecutor(
				CommandLib.SETTINGS,
				new ClientPacketLittleBlocksLoginExecutor());

		handler.getPacketHandler(PacketIds.UPDATE).registerClientExecutor(
				CommandLib.UPDATE_CLIENT, new ClientBlockChangeExecutor());

		handler.getPacketHandler(PacketIds.ENTITY).registerClientExecutor(
				CommandLib.ENTITY_COLLECTION,
				new ClientLittleCollectionExecutor());

		handler.getPacketHandler(PacketIds.PLAYER).registerClientExecutor(
				CommandLib.COPIER_MESSAGE, new ClientCopierNotifyExecutor());

		handler.getPacketHandler(PACKETID_EVENT).registerClientExecutor(
				CommandLib.BLOCK_EVENT, new ClientBlockEventExecutor());
	}
	...
	@SideOnly(Side.CLIENT)
	public static void blockActivated(World world, EntityPlayer entityplayer,
			int x, int y, int z, int q, float a, float b, float c) {
		PacketLittleBlock packetLB = new PacketLittleBlock(x, y, z, q,
				entityplayer.getCurrentEquippedItem(), a, b, c);
		PacketHelper.sendToServer(packetLB);
	}

	public static void sendBlockEvent(int x, int y, int z, int blockID,
			int eventID, int eventParameter) {
		PacketLittleBlocksEvents eventPacket = new PacketLittleBlocksEvents(x,
				y, z, blockID, eventID, eventParameter);
	...
	public static void sendBlockPlace(World world, EntityPlayer entityplayer,
			int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		PacketLittleBlock placePacket = new PacketLittleBlock(x, y, z, side,
				entityplayer.inventory.getCurrentItem(), hitX, hitY, hitZ);
		PacketHelper.sendToServer(placePacket);
	}

	public static void sendItemUse(World world, EntityPlayer entityplayer,
			int x, int y, int z, int side, ItemStack itemstack) {
		PacketLittleBlock usePacket = new PacketLittleBlock(x, y, z, side,
				itemstack, 0.0f, 0.0f, 0.0f);
		PacketHelper.sendToServer(usePacket);
	}

	public static void sendBlockChange(World world, EntityPlayer entityplayer,
			int x, int y, int z) {
	...
	public static void wandModeSwitched() {
		PacketLittleNotify packet = new PacketLittleNotify(
				CommandLib.WAND_SWITCH);
		PacketHelper.sendToServer(packet);
	}

	public static void sendWandChange(World world, EntityPlayer entityplayer,
			int actionID) {
		PacketLittleNotify wandPacket = new PacketLittleNotify(
				CommandLib.WAND_SWITCH);
