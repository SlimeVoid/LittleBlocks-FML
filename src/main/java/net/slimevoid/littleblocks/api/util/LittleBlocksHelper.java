...
public class LittleBlocksHelper implements ISlimevoidHelper {
	...
	public static void init(ICommonProxy littleProxy, int littleBlocksSize) {
		if (!initialized) {
			ISlimevoidHelper littleBlocksHelper = new LittleBlocksHelper(
					littleProxy, littleBlocksSize);
	...
	private IBlockAccess getWorld(IBlockAccess world, int x, int y, int z) {
		if (isLittleBlock(world, x, y, z)) {
			return (World) ((ILBCommonProxy) proxy)
	...
	public boolean isUseableByPlayer(World world, EntityPlayer player,
		...
		if (isLittleBlock(world, xCoord, yCoord, zCoord)) {
			return player.getDistanceSq((xCoord / size) + xDiff,
					(yCoord / size) + yDiff, (zCoord / size) + zDiff) <= distance;
		}
		return false;
	}
	...
	public boolean isLadder(IBlockAccess world, int x, int y, int z,
		...
		if (((World) world).checkChunksExist(MathHelper.floor_double(minX),
			...
			// is 0
			minX = MathHelper.floor_double((minX - MathHelper
					.floor_double(minX)) * tile.size)
					+ ((MathHelper.floor_double(minX) - x) * tile.size);
			minY = MathHelper.floor_double((minY - MathHelper
					.floor_double(minY)) * tile.size)
					+ ((MathHelper.floor_double(minY) - y) * tile.size);
			minZ = MathHelper.floor_double((minZ - MathHelper
					.floor_double(minZ)) * tile.size)
					+ ((MathHelper.floor_double(minZ) - z) * tile.size);
			maxX = MathHelper.floor_double((maxX - MathHelper
					.floor_double(maxX)) * tile.size)
					+ ((MathHelper.floor_double(maxX) - x) * tile.size);
			maxZ = MathHelper.floor_double((maxZ - MathHelper
					.floor_double(maxZ)) * tile.size)
				...
				for (int littleZ = (int) minZ; littleZ <= maxZ; littleZ++) {
					...
					if (block.getMaterial() != Material.air) {
						int xx = (x << 3) + littleX, yy = (y << 3) + (int) minY, zz = (z << 3)
								+ littleZ;
						if (block.isLadder((World) tile.getLittleWorld(), xx,
								yy, zz, entity)) {
							return true;
						}
					}
