...
public class ItemInLittleWorldManager extends ItemInWorldManager {
	...
	public void onBlockClicked(int x, int y, int z, int side) {
		PlayerInteractEvent event = new PlayerInteractEvent(this.thisPlayerMP,
				Action.LEFT_CLICK_BLOCK, x, y, z, side);
		MinecraftForge.EVENT_BUS.post(event);
		if (event.isCanceled()) {
			PacketLib
					.sendBlockChange(this.theWorld, this.thisPlayerMP, x, y, z);
			return;
		}
		if (this.isCreative()) {
			...
			if (event.useBlock != Event.Result.DENY) {
				...
				PacketLib.sendBlockChange(this.theWorld, this.thisPlayerMP, x,
						y, z);
			}
	...
	public boolean tryHarvestBlock(int x, int y, int z) {
		...
		if (this.isCreative()) {
			...
			if (playerHeldItem != null) {
				playerHeldItem.func_150999_a/* onBlockDestroyed */(
						this.theWorld, block, x, y, z, this.thisPlayerMP);
			}

			blockHarvested = this.removeBlock(x, y, z);
			if (blockHarvested && canHarvest) {
				block.harvestBlock(this.theWorld, this.thisPlayerMP, x, y, z,
						metadata);
			}
		}
	...
	public boolean activateBlockOrUseItem(EntityPlayer entityplayer,
			World world, ItemStack itemstack, int x, int y, int z, int side,
			float hitX, float hitY, float hitZ) {
		PlayerInteractEvent event = ForgeEventFactory.onPlayerInteract(
				entityplayer, Action.RIGHT_CLICK_BLOCK, x, y, z, side);
		if (event.isCanceled()) {
			PacketLib
					.sendBlockChange(this.theWorld, this.thisPlayerMP, x, y, z);
			return false;
		...
		if (item != null
				&& item.onItemUseFirst(itemstack, entityplayer, world, x, y, z,
						side, hitX, hitY, hitZ)) {
			if (itemstack.stackSize <= 0)
				ForgeEventFactory.onPlayerDestroyItem(thisPlayerMP, itemstack);
			return true;
		}
		...
		if (block != null
				&& (!entityplayer.isSneaking() || (entityplayer.getHeldItem() == null || entityplayer
						.getHeldItem().getItem()
						.doesSneakBypassUse(world, x, y, z, entityplayer)))) {
			if (event.useBlock != Event.Result.DENY) {
				result = block.onBlockActivated(world, x, y, z, entityplayer,
						side, hitX, hitY, hitZ);
			} else {
				PacketLib.sendBlockChange(this.theWorld, this.thisPlayerMP, x,
						y, z);
		...
		if (itemstack != null && !result && event.useItem != Event.Result.DENY) {
			...
			result = itemstack.tryPlaceItemIntoWorld(entityplayer, world, x, y,
