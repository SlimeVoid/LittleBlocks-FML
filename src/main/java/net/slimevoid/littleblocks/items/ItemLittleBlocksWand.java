...
public class ItemLittleBlocksWand extends Item {
	...
	public boolean onItemUseFirst(ItemStack itemstack,
		...
		if (entityplayer.canPlayerEdit(x, y, z, l, itemstack)) {
			if (!world.isRemote) {
				EnumWandAction playerWandAction = EnumWandAction
						.getWandActionForPlayer(entityplayer);
				if (playerWandAction.equals(EnumWandAction.COPY_LB)) {
					return this.doCopyLB(itemstack, entityplayer, world, x, y,
							z, l, a, b, c);
				}
				if (playerWandAction.equals(EnumWandAction.ROTATE_LB)) {
					return this.doRotateLB(itemstack, entityplayer, world, x,
							y, z, l, a, b, c);
				}
				if (playerWandAction.equals(EnumWandAction.PLACE_LB)) {
					return this.doPlaceLB(itemstack, entityplayer, world, x, y,
							z, l, a, b, c);
				}
			}
		}
	...
	private boolean doRotateLB(ItemStack itemstack, EntityPlayer entityplayer,
		...
		if (world.getBlock(x, y, z) == ConfigurationLib.littleChunk) {
			((BlockLittleChunk) ConfigurationLib.littleChunk)
					.rotateLittleChunk(world, x, y, z, ForgeDirection.UP);
			return true;
		}
		return false;
	}
	...
	private boolean doPlaceLB(ItemStack itemstack, EntityPlayer entityplayer,
		...
		if (blockID != ConfigurationLib.littleChunk) {
			...
			if (blockID.getMaterial() == Material.air
					|| blockID.isAir(world, x, y, z)
					|| blockID.isReplaceable(world, x, y, z)
					&& !(world.getBlock(x, y, z) instanceof IFluidBlock)) {
	...
	private boolean doCopyLB(ItemStack itemstack, EntityPlayer entityplayer,
			World world, int x, int y, int z, int l, float a, float b, float c) {
								...
								for (int y1 = 0; y1 < ConfigurationLib.littleBlocksSize; y1++) {
									for (int z1 = 0; z1 < ConfigurationLib.littleBlocksSize; z1++) {
										if (oldtile.getBlock(x1, y1, z1) != Blocks.air) {
											Block blockId = oldtile.getBlock(
													x1, y1, z1);
											int metadata = oldtile
													.getBlockMetadata(x1, y1,
															z1);
											newtilelb.setBlockIDWithMetadata(
													x1, y1, z1, blockId,
													metadata);
											TileEntity oldLittleTile = oldtile
													.getChunkTileEntity(x1, y1,
															z1);
		...
		PacketHelper.sendToPlayer(new PacketLittleNotify(
				CommandLib.COPIER_MESSAGE), (EntityPlayerMP) entityplayer);
		return false;
	}
}
