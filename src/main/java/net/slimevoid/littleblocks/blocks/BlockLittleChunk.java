...
public class BlockLittleChunk extends BlockContainer {
	...
	public BlockLittleChunk(int id, Class<? extends TileEntity> clazz,
			Material material, float hardness, boolean selfNotify) {
		super(material);
		this.clazz = clazz;
	...
	public Item getItem(World world, int x, int y, int z) {
		...
		if (tileentity != null && tileentity instanceof TileEntityLittleChunk) {
			...
			Block pickedBlock = tilelb
					.getBlock(xSelected, ySelected, zSelected);
	...
	@Override
	public boolean removedByPlayer(World world, EntityPlayer entityplayer,
			int x, int y, int z) {
		...
		if (id == ConfigurationLib.littleChunk) {
			...
			EntityItemLittleBlocksCollection collection = new EntityItemLittleBlocksCollection(
					world, x, y, z, new ItemStack(ConfigurationLib.littleChunk));
			if (!tile.isEmpty()) {
			...
			if (!world.isRemote) {
				if (!collection.isEmpty()) {
					...
					PacketLittleBlocksCollection packet = new PacketLittleBlocksCollection(
							collection);
	...
	private ItemStack dropLittleBlockAsNormalBlock(World world, int x, int y,
			int z, Block blockId, int metaData) {
		...
		if (dropsBlocks) {
			...
			ItemStack itemstack = new ItemStack(idDropped, quantityDropped,
					damageDropped);
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
			EntityPlayer entityplayer, int localSide, float hitX, float hitY,
			float hitZ) {
			...
			if (this.canPlayerPlaceBlockOrUseItem(world, entityplayer)) {
				try {
					BlockUtil.getLittleController().onPlayerRightClickFirst(
							...
							((x << 3) + xSelected), ((y << 3) + ySelected),
							((z << 3) + zSelected), side, hitX, hitY, hitZ);
	...
	private boolean canPlayerPlaceBlockOrUseItem(World world,
			...
			if (item != null) {
				if (item instanceof ItemBlock) {
					...
					if (itemBlockId.hasTileEntity(0)) {
						if (!BlockUtil.isTileEntityAllowed(itemBlockId
		...
		if (denyPlacement) {
			entityplayer.addChatMessage(ChatHelper.getMessage(
					EnumChatFormatting.BOLD, placementMessage));
			return false;
		}
		return true;
	}

	public void onServerBlockActivated(World world, EntityPlayer entityplayer,
			int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
		if (!entityplayer.canPlayerEdit(x >> 3, y >> 3, z >> 3, side,
		...
		if (this.canPlayerPlaceBlockOrUseItem(world, entityplayer)) {
			try {
				BlockUtil.onServerBlockActivated(world, entityplayer,
						entityplayer.getCurrentEquippedItem(), x, y, z, side,
						hitX, hitY, hitZ);
			} catch (ClassCastException e) {
				FMLCommonHandler.instance().getFMLLogger()
						.warn(e.getLocalizedMessage());
	...
	public void onBlockClicked(World world, int x, int y, int z,
		...
		if (world.isRemote) {
			BlockUtil.getLittleController().clickBlock((x << 3) + xSelected,
					(y << 3) + ySelected, (z << 3) + zSelected, side);
		}
	}

	public void onServerBlockClicked(World world, int x, int y, int z,
			int side, EntityPlayer entityplayer) {
		if (entityplayer instanceof EntityPlayerMP) {
			EntityPlayerMP player = (EntityPlayerMP) entityplayer;
			if (!FMLCommonHandler
					.instance()
					.getMinecraftServerInstance()
					.isBlockProtected(((ILittleWorld) world).getParentWorld(),
							x >> 3, y >> 3, z >> 3, entityplayer)) {
				BlockUtil.getLittleItemManager(player, world).onBlockClicked(x,
						y, z, side);
			} else {
	...
	public boolean rotateLittleChunk(World world, int x, int y, int z,
			ForgeDirection axis) {
		TileEntity tileentity = world.getTileEntity(x, y, z);
		if (tileentity != null && tileentity instanceof TileEntityLittleChunk) {
	...
	private void setBlockBoundsBasedOnSelection(World world, int x, int y, int z) {
		...
		if (xSelected == -10) {
			...
			if (block == null) {
				this.setBlockBounds(xSelected / m, ySelected / m,
						zSelected / m, (xSelected + 1) / m,
						(ySelected + 1) / m, (zSelected + 1) / m);
			} else {
				if (block != null) {
					...
					this.setBlockBounds(
							(float) (xSelected + block.getBlockBoundsMinX())
									/ m,
	...
	public void addCollisionBoxesToList(World world, int x, int y, int z,
		...
		if (tileentity != null && tileentity instanceof TileEntityLittleChunk) {
			...
			AxisAlignedBB bb = AxisAlignedBB.getBoundingBox(axisalignedbb.minX
					* m, axisalignedbb.minY * m, axisalignedbb.minZ * m,
					axisalignedbb.maxX * m, axisalignedbb.maxY * m,
			...
			for (AxisAlignedBB aabb : bbs) {
				aabb.setBounds(aabb.minX / m, aabb.minY / m, aabb.minZ / m,
						aabb.maxX / m, aabb.maxY / m, aabb.maxZ / m);
				list.add(aabb);
			}
		}
	}
	...
	public MovingObjectPosition collisionRayTrace(World world, int x, int y,
		...
		returns = CollisionRayTrace.rayTraceLittleBlocks(this, player, view, x,
				y, z, returns, tile, isFluid);
		player = player.addVector(-x, -y, -z);
		view = view.addVector(-x, -y, -z);

		returns = CollisionRayTrace.collisionRayTracer(this, world, player,
				view, x, y, z, returns);
		if (!returns.isEmpty()) {
			...
			for (MovingObjectPosition ret : returns) {
				...
				Block retBlock = tile.getBlock(ret.blockX, ret.blockY,
						ret.blockZ);
				if (retBlock != Blocks.air) {
					isLiquid = retBlock instanceof IFluidBlock;
					if (isLiquid && isFluid) {
						isLiquid = !(retBlock instanceof BlockStaticLiquid && tile
								.getBlockMetadata(ret.blockX, ret.blockY,
										ret.blockZ) == 0);
					}
				}
			...
			if (min != null) {
				...
				if (littleBlock != Blocks.air) {
					if (littleBlock != null) {
						if (!(littleBlock.hasTileEntity(tile.getBlockMetadata(
								xSelected, ySelected, zSelected)) && tile
								.getChunkTileEntity(xSelected, ySelected,
										zSelected) == null)) {
				...
				hitVec = min.hitVec.myVec3LocalPool.getVecFromPool(
	...
	public MovingObjectPosition rayTraceBound(AxisAlignedBB bound, int i,
			int j, int k, Vec3 player, Vec3 view) {
		...
		if (minX != null
				&& (tracedBound == null || player.squareDistanceTo(minX) < player
						.squareDistanceTo(tracedBound))) {
			tracedBound = minX;
		}
		if (maxX != null
				&& (tracedBound == null || player.squareDistanceTo(maxX) < player
						.squareDistanceTo(tracedBound))) {
			tracedBound = maxX;
		}
		if (minY != null
				&& (tracedBound == null || player.squareDistanceTo(minY) < player
						.squareDistanceTo(tracedBound))) {
			tracedBound = minY;
		}
		if (maxY != null
				&& (tracedBound == null || player.squareDistanceTo(maxY) < player
						.squareDistanceTo(tracedBound))) {
			tracedBound = maxY;
		}
		if (minZ != null
				&& (tracedBound == null || player.squareDistanceTo(minZ) < player
						.squareDistanceTo(tracedBound))) {
			tracedBound = minZ;
		}
		if (maxZ != null
				&& (tracedBound == null || player.squareDistanceTo(maxZ) < player
						.squareDistanceTo(tracedBound))) {
			tracedBound = maxZ;
		}
		if (tracedBound == null) {
				...
				for (int side = 0; side < 6; side++) {
					...
					for (int xx = startX; xx < maX; xx++) {
						for (int yy = startY; yy < maY; yy++) {
							for (int zz = startZ; zz < maZ; zz++) {
								if (tile.getBlock(xx, yy, zz) != Blocks.air) {
									Block littleBlock = tile.getBlock(xx, yy,
											zz);
									if (littleBlock != null) {
	...
	@Override
	public boolean addDestroyEffects(World world, int x, int y, int z,
			int meta, EffectRenderer effectRenderer) {
		return LittleBlockDiggingFX.doBlockDestroyEffects(world, x, y, z, meta,
				effectRenderer, this);
	}
	...
	public boolean addHitEffects(World world, MovingObjectPosition target,
		...
		return LittleBlockDiggingFX.doBlockHitEffects(world, target,
				effectRenderer, this);
	}
