...
public class BlockLBPistonBase extends BlockPistonBase {
	 ...
	 */
	public static boolean onEventReceived(World world, int x, int y, int z,
			int blockID, int eventID) {
		...
		if (world instanceof ILittleWorld) {
			if (((ILittleWorld) world).isOutSideLittleWorld(xOffset, yOffset,
					zOffset)) {
				return false;
			}
		}
		return Blocks.piston.onBlockEventReceived(world, x, y, z, blockID,
				eventID);
	}
}