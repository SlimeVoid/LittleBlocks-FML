...
public class PistonOrientation {
	...
	public void onPistonPlacement(BlockOrientationEvent event) {
		LittleBlockCoordinates coords = new LittleBlockCoordinates(event.blockX, event.blockY, event.blockZ);
		if (LittleBlockDataHandler.isLittleWorld(event.world)) {
			LittleBlockDataHandler.shiftCoordsRight(coords, 3);
			event.blockX = coords.x;
